...
int main(int argc, char **argv) {
        ...
        // &core_ID_timer); while (ticks == 0) {} printf("Started Clock
        ...
        int total_num_of_pointers =
            num_of_train_pointers + num_of_test_pointers;
        ...
        for (int i = 0; i < total_num_of_pointers; i++) {
                ...
                // Make all of the test pointers the pointer we are trying to
                // guess
        ...
        fprintf(stdout,
                "[+] we've set the %u pointers after the first %u train "
                "pointers to:\n",
                total_num_of_pointers - num_of_train_pointers,
                num_of_train_pointers);
        ...
        // Allocate a large array (12 times the cache) that we can access to
        // flush the entire cache. High performance cores: 12MB of shared L2
        // cache, 192KB L1 instruction cache, and 128KB L1 data cache. High
        // efficiency cores: 4MB of shared L2 cache, 128KB L1 instruction cache,
        // and 64KB L1 data cache.
        ...
        for (size_t ptr_no = 0; ptr_no < 3; ptr_no++) {
                // Set test_p to either test1, test2, or test3 depending on what
                // ptr_no is
                ...
                for (uint32_t i = 0; i < repetitions; i++) {
                        ...
                        // Thrash the cache
                        for (uint32_t j = 0;
                             j < size_of_thrash_array / sizeof(uint64_t) - 2;
                             j++) {
                                __trash += (thrash_arr[j] ^ __trash) & 0b1111;
                                __trash +=
                                    (thrash_arr[j + 1] ^ __trash) & 0b1111;
                                __trash +=
                                    (thrash_arr[j + 2] ^ __trash) & 0b1111;
                        }
                        ...
                        __trash += MSB_MASK &
                                   *((test_p + offset_const) + (T1 & MSB_MASK) +
                                     (__trash & MSB_MASK));
