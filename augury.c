...
static void sighandler_local() {
        ...
        assert(0 && "Process changed cores; PMC no longer available."
                    ...
                    "If this happens too often, consider using the MACH timer "
                    "instead.");
...
#warning "Using threaded timer"
        ...
        pthread_create(&timer_thread, NULL, clock_thread,
                       (void *)&core_ID_timer);
        while (ticks == 0) {
        }
...
#warning "Forcing data_buffer address"
        ...
        volatile uint64_t *data_buffer = mmap(
            (void *)data_base_addr, size_of_data_array, PROT_READ | PROT_WRITE,
            MAP_FIXED | MAP_ANON | MAP_PRIVATE, -1, 0);

        /* assert((uintptr_t)data_buffer > (uintptr_t)(data_base_addr - (2 <<
         * 24)));
         */
        /* assert((uintptr_t)data_buffer < (uintptr_t)(data_base_addr + (2 <<
         * 24)));
...
#ifdef MACH
                T1 = get_time_nano((__trash + test_idx_in_data_buffer) &
                                   MSB_MASK);
#else
#ifdef KEXT
                T1 = READ_TIMER;
#else  // threaded timer
                T1 = *(&ticks +
                       ((__trash + test_idx_in_data_buffer) & MSB_MASK) +
                       (train_time & MSB_MASK));
#endif // KEXT
#endif // MACH
